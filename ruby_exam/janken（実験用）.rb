require 'io/console'
# プレイヤー(自分)に「1~3」を入力させるロジックを書きます。
class Player
  def hand
    puts "数字を入力してください。"
    puts "[1]:グー\n[2]:チョキ\n[3]:パー"
    while true                                  #「input_hand」が「0, 1, 2」のいずれかだと繰り返し処理を終了し、それ以外（アルファベットも含む）だと繰り返し処理を継続する。
    @input_hand = STDIN.noecho(&:gets).to_i                    #ヒント：include?メソッドについて調べてみましょう。　　　　⇒使うのか？     [STDIN.noecho(&:gets).to_i]
      if @input_hand == 1                        #if～elsif「input_hand」が「0, 1, 2」のいずれかの場合だった場合
        puts "[1]:グー"                           #「input_hand」をそのまま返してもよかったが、一応手も表記。
        return false                            #ヒント：戻り値を返して繰り返し処理を終了させたい場合、「return」を使用します。たぶん省略可能。
      elsif @input_hand == 2
        puts "[2]:チョキ"
        return false
      elsif @input_hand == 3
        puts "[3]:パー"
        return false
      else                                      #else それ以外の場合
        puts "てめえの頭はハッピーセットかよ。"
        puts "1〜3の中から選ぶんだ。半角だぞ。"
        puts "[1]:グー\n[2]:チョキ\n[3]:パー"
      end                                       # end if文のend
    true
    end
  end
end   

class Enemy                     # 相手が「1~3」の値をランダムに生成するロジックを書きます。
  def hand
    @enemy_hand =rand(3).to_i    # グー、チョキ、パーの値をランダムに取得する。
      if @enemy_hand == 0                        #if～elsif「input_hand」が「0, 1, 2」のいずれかの場合だった場合
        puts "[1]:グー"                           #「input_hand」をそのまま返してもよかったが、一応手も表記。
        return false                            #ヒント：戻り値を返して繰り返し処理を終了させたい場合、「return」を使用します。たぶん省略可能。
      elsif @enemy_hand == 1
        puts "[2]:チョキ"
        return false
      elsif @enemy_hand == 2
        puts "[3]:パー"
        return false
      else                                      #else それ以外の場合
        puts "てめえの頭はハッピーセットかよ。"
        puts "1〜3の中から選ぶんだ。"
        puts "[1]:グー\n[2]:チョキ\n[3]:パー"
      end                                       # end if文のend
  end
end

class Janken                     # プレイヤー(自分)が入力した「0~2」と、敵がランダムで生成した「0~2」をじゃんけんをさせて、その結果をコンソール上に出力するロジックを書きます。
  def pon(player_hand, enemy_hand)
    player_hand = @input_hand.to_i
    enemy_hand = @enemy_hand.to_i
    janken = ["グー", "チョキ", "パー"]
    puts "相手の手は#{janken[enemy_hand.to_i]}です。"      #なぜかいつもグーになる。おそらく数として認識されず文字として認識されているのだとはおもうが・・・なぜ？
                                                          #あと、なぜ@を使ったPlayerクラスのインスタンス変数がEnemyクラスやJankenクラスにも使えているのか・・・？
  end
  
  def result                                    # Playerクラスの戻り値とEnemyクラスの戻り値からじゃんけんするロジックを作成します。
    if @player_hand - @enemy_hand == 0             # Playerクラスの戻り値(player_hand)とEnemyクラスの戻り値(enemy_hand)の値が同じだった場合
      puts "あいこ"                              #「あいこ」を出力します。
      true                                      #「true」を返してじゃんけんを繰り返し実行させます。ヒント：「return」を使って戻り値を返すことができます。しかし、Rubyでは戻り値を返す場合、「return」を省略するのが一般的。
    elsif (@player_hand + 3 - @enemy_hand)%3 ==2  #もしくは (player_hand == 0 && enemy_hand == 1) || (player_hand == 1 && enemy_hand == 2) || (player_hand == 2 && enemy_hand == 0)
      puts "あなたの勝ちです"                    #「あなたの勝ちです」を出力します。
      false                                     #「false」を返してじゃんけんを終了させます。
    else
      puts "あなたの負けです"                    #「あなたの負けです」を出力します。
      false                                     #「false」を返してじゃんけんを終了させます。
    end
  end
end

class GameStart                   # じゃんけんゲームを実行するロジックを書く。
  def self.jankenpon              # selfを使用すると、GameStartをインスタンス化することなく、クラス名を使ってjankenponメソッドを呼び出せる。
    player = Player.new# 変数「player」にPlayerをインスタンス化したものを代入します。
    enemy = Enemy.new    # 変数「enemy」にEnemyをインスタンス化したものを代入します。
    janken = Janken.new    # 変数「janken」にJankenをインスタンス化したものを代入します。
                                    #なぜ、Player.handやJanken.resultを実行しなくてもきちんと実行されるのか
    next_game = janken

    while next_game == true
      if next_game
        true
      else next_game
        false
      end      
    end
    # 「next_game」が「false」だと繰り返し処理を終了し、「true」だと繰り返し処理を継続します。
    while next_game
      # 変数「next_game」にじゃんけんを実行して返ってきた値(戻り値)を代入します。
      #「janken.pon(player.hand, enemy.hand)」でじゃんけんを実行しています。
      next_game = janken.pon(player.hand, enemy.hand)
    end
  end
end

gamestart = GameStart.new
gamestart = GameStart.jankenpon